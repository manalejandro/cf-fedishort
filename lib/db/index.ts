...
function rowToShortLink(r: Row): LocalShortLink {
  return {
    id: r.id,
    slug: r.slug,
    actorId: r.actor_id,
...
export async function updateActorCounts(
  db: D1Database,
  id: string,
  counts: { followersCount?: number; followingCount?: number; linksCount?: number }
): Promise<void> {
  const sets: string[] = [];
  ...
  if (counts.linksCount !== undefined) { sets.push("links_count = ?"); vals.push(counts.linksCount); }
...
export async function getShortLinksByActor(db: D1Database, actorId: string): Promise<LocalShortLink[]> {
  const { results } = await db
    .prepare("SELECT * FROM short_links WHERE actor_id = ? ORDER BY published DESC")
    .bind(actorId)
    .all<Row>();
  return results.map(rowToShortLink);
}

export async function getShortLinkBySlug(db: D1Database, slug: string): Promise<LocalShortLink | null> {
  const row = await db.prepare("SELECT * FROM short_links WHERE slug = ?").bind(slug).first();
...
export async function createShortLink(
  ...
  await db
    .prepare(
      "INSERT INTO short_links (id, slug, actor_id, url, title, description, object_id, published, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, COALESCE(?, datetime('now')), datetime('now'))"
    )
    .bind(link.id, link.slug, link.actorId, link.url, link.title ?? null, link.description ?? null, link.objectId ?? null, link.published ?? null)
    .run();
}

export async function incrementLinkClicks(db: D1Database, linkId: string): Promise<void> {
  await db
    .prepare("UPDATE short_links SET clicks = clicks + 1 WHERE id = ?")
    .bind(linkId)
    .run();
}

export async function getShortLinksByActorPaginated(
  db: D1Database,
  actorId: string,
  limit = 20,
  offset = 0
): Promise<LocalShortLink[]> {
  const { results } = await db
    .prepare("SELECT * FROM short_links WHERE actor_id = ? ORDER BY published DESC LIMIT ? OFFSET ?")
    .bind(actorId, limit, offset)
    .all<Row>();
  return results.map(rowToShortLink);
}

export async function getShortLinkById(db: D1Database, id: string): Promise<LocalShortLink | null> {
  const row = await db.prepare("SELECT * FROM short_links WHERE id = ?").bind(id).first();
  return row ? rowToShortLink(row) : null;
}

export async function deleteShortLink(db: D1Database, id: string): Promise<void> {
